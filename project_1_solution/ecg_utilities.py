...
# -----------------------------------------------------------------------------
# R-peak and IBI detection from an ECG signal
# -----------------------------------------------------------------------------
...
def moving_average(ecg, n=10):
    ...
    Returns:
        Filtered ecg time-series
    """
    return numpy.convolve(ecg, numpy.ones((n,)) / n, mode='valid')


def detect_r_peaks(ecg, fs=500):
    """ Calculate the IBI vector from the ECG time-series

    Args:
        ecg: ecg time-series
        fs: samping rate
    Returns:
        A vector containing the inter-beat intervals of the input ecg
    """
